...
class VisualBertAddingFusionEmbeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings and visual embeddings."""
    # concat fusion of text embedding and visual embedding
    def __init__(self, config):
        super().__init__()
        self.config = config
        # **Do not** need word embeddings for adding fusion for current stage
        # but keep this for model get_embedding and set_embedding function
        ...
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size) if config.use_token_type else None
    ...
    def forward(
        ...
        if position_ids is None:
            # whether use cls token
            position_ids = self.position_ids[:, :seq_length+1] if self.config.use_cls_token else self.position_ids[:, :seq_length]

        if inputs_embeds is None:
            inputs_embeds = self.word_embeddings(input_ids)

        assert visual_embeds is not None, "Adding fusion for text embedding and visual embedding, but visual embedding is None."
        embeddings = inputs_embeds + self.visual_projection(visual_embeds)
        ...
        fwd_return.embeddings = embeddings
        if return_pos_tok:
            fwd_return.position_embeddings = position_embeddings
            fwd_return.token_type_embeddings = token_type_embeddings

        return fwd_return


class VisualBertVTConcatEmbeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings and visual embeddings."""
    # concat fusion of text embedding and visual embedding
    def __init__(self, config):
        super().__init__()
    ...
    def forward(
        ...
        # concat text embedding and visual embedding
        position_ids = self.position_ids[:, :2*seq_length+1] if self.config.use_cls_token else self.position_ids[:, :2*seq_length]


        assert visual_embeds is not None, "VT Concat fusion for text embedding and visual embedding, but visual embedding is None."
        embeddings = torch.cat((inputs_embeds , self.visual_projection(visual_embeds)), dim=1)
        
        if self.token_type_embeddings is not None:
            text_token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=self.position_ids.device)
            vis_token_type_ids = torch.ones(input_shape,dtype=torch.long, device=self.position_ids.device)
            token_type_ids = torch.cat((text_token_type_ids, vis_token_type_ids), dim=1)
            token_type_embeddings = self.token_type_embeddings(token_type_ids)
            embeddings += token_type_embeddings
        ...
        # Absolute Position Embeddings
        position_embeddings = self.position_embeddings(position_ids)
        if position_embeddings.shape[0]!=embeddings.shape[0]:
...
class VisualBertQueryConcatEmbeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings and visual embeddings."""
    # concat fusion of text embedding and visual embedding
    def __init__(self, config):
        super().__init__()
    ...
    def forward(
        ...
        fwd_return.embeddings = embeddings
        if return_pos_tok:
            # should be fixed to be more robust
            # import ipdb;ipdb.set_trace()
            fwd_return.position_embeddings = torch.cat(
                (position_embeddings.expand(visual_position_embeddings.shape[0],-1,-1), visual_position_embeddings),
                dim=1
            )
            fwd_return.token_type_embeddings = torch.cat(
                (token_type_embeddings, visual_token_type_embeddings),
                dim=1
            )
...
class VisualBertModel(VisualBertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        # added by Stan
        TYPE2EMB = edict(
            query_concat = VisualBertQueryConcatEmbeddings,
            adding_fusion = VisualBertAddingFusionEmbeddings,
            vt_concat = VisualBertVTConcatEmbeddings
        )
        # self.embeddings = VisualBertEmbeddings(config)
        if config.embedding_type not in TYPE2EMB.keys():
            raise NotImplementedError("Embedding Type {} not implemented.".format(config.embedding_type))
        self.embeddings = TYPE2EMB[config.embedding_type](config)

        self.fwd_embeddings_outp = None

        self.combined_attn_mask = None
    ...
    def forward(
        ...
        # Added by Stan: concat | adding
        if visual_embeds is not None:
            if self.config.embedding_type in ["query_concat", "vt_concat"]:
                combined_attention_mask = torch.cat((attention_mask, visual_attention_mask), dim=-1)
                shape = input_shape + visual_input_shape
            elif self.config.embedding_type == "adding_fusion":
            ...
            if self.config.use_cls_token:
                combined_attention_mask = torch.cat(
                    (
                        torch.ones((combined_attention_mask.shape[0] ,1), 
            ...
            # added by stan for fusion
            self.combined_attn_mask = combined_attention_mask

            extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(
                # should be fixed when using decoder
                combined_attention_mask, [batch_size, shape], device
            )
        ...
        #### case for embedding type #####
        if self.config.embedding_type == "query_concat":
            self.fwd_embeddings_outp = self.embeddings(
                input_ids=input_ids,
                position_ids=position_ids,
                token_type_ids=token_type_ids,
                inputs_embeds=inputs_embeds,
        ...
        elif self.config.embedding_type == "vt_concat":
            self.fwd_embeddings_outp = self.embeddings(
                inputs_embeds = inputs_embeds,
                visual_embeds = visual_embeds,
                return_pos_tok = return_pos_tok
        ...
        embedding_output = self.fwd_embeddings_outp.embeddings
        ###############
